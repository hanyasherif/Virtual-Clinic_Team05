...
const FilterAppointmentsPatient = () => { 
  ...
  const getDoctorName = async (doctorId) => {
    try {
      const response = await axios.get(`http://localhost:4000/getDoctorName/${doctorId}`);
  ...
  const fetchDoctorNames = async () => {
    try {
      const doctorNames = {};
      const appointments = await axios.get('http://localhost:4000/viewAppointments');
  ...
  useEffect(() => {
    ...
    const getAppointment=  async () => {
        ...
        */     
        ...
        await axios.get('http://localhost:4000/viewAppointments').then(
            (res) => { 
    ...
    const getAppointmentDate=  async (date) => {
      ...
      */     
      ...
      await axios.get(`http://localhost:4000/filterAppointmentsDate/${date}`).then(
          (res) => { 
              const appointments = res.data
  ...
  const getAppointmentStatus=  async (status) => {
    ...
    */     
    ...
    await axios.get(`http://localhost:4000/filterAppointmentsStatus/${status}`).then(
        (res) => { 
            const appointments = res.data
