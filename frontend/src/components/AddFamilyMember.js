...
const AddFamilyMember = () => {
   ...
   const id = '65735cebad66db980718a14d';
    ...
    const assignFamilyMemberValues = async (username) => {
      try {
        const response = await fetch(`/getUserByUsername/${username}`);
    ...
    const fetchUser = async (email, phoneNumber) => {
      if(email){
      try {
        const response = await axios.get(`http://localhost:8000/getUserByEmail/${email}`);
      ...
      try {
        const response = await axios.get(`http://localhost:8000/getUserByPhoneNumber/${phoneNumber}`);
